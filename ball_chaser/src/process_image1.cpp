...
{
    int white_pixel = 255;
    int img_sec = img.width/3;     // Divide the image into 3 sections
    bool ball_not_detected = false;         
    ...
    {
        if (img.data[i] == white_pixel && img.data[(i) + 1]) 
        {
            // Calculate pixel row position
            int j = i % img.width;
            if(j > img_sec * 2)       // Ball detected to the right
            {
                drive_robot(0.0, -0.5);
            }else if (j > img_sec)    // Ball detected in the middle 
            {
                drive_robot(0.5, 0.0);
